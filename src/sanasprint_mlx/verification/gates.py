...
def build_verification_report(
    *,
    ...
    gates["real_block_denoise"] = _real_block_denoise_gate(snapshot, evidence)
    gates["real_transformer_loop"] = _real_transformer_loop_gate(snapshot, evidence)
...
def _scaffold_denoise_gate(snapshot, allow_download: bool, evidence: dict) -> dict:
    del allow_download
    gate_id = "scaffold_denoise"
    command = (
        "sanasprint-mlx-verify scaffold-denoise "
        f"--snapshot {snapshot or '/path/to/Sana_Sprint_0.6B_1024px_diffusers'} "
        "--output /tmp/sanasprint-mlx-scaffold-denoise.json "
        "--dtype bfloat16 --steps 1"
    )
    if gate_id in evidence["gates"]:
        item = dict(evidence["gates"][gate_id])
        return {"id": gate_id, "status": "PASS", "reason": "external pass evidence", "command": command, **item}
    ...
    if _looks_remote(snapshot):
        return {
            "id": gate_id,
            "status": "BLOCKED",
            "reason": "scaffold denoise requires a local snapshot path",
            "command": command,
        }
    exists = Path(snapshot).exists()
    return {
...
def _block0_attention_gate(snapshot, evidence: dict) -> dict:
    gate_id = "block0_attention"
    command = (
        "sanasprint-mlx-verify block0-attention "
        f"--snapshot {snapshot or '/path/to/Sana_Sprint_0.6B_1024px_diffusers'} "
        "--output /tmp/sanasprint-mlx-block0-attention.json "
        "--dtype bfloat16"
    )
    if gate_id in evidence["gates"]:
        item = dict(evidence["gates"][gate_id])
...
def _real_block_denoise_gate(snapshot, evidence: dict) -> dict:
    gate_id = "real_block_denoise"
    command = (
        "sanasprint-mlx-verify real-block-denoise "
        f"--snapshot {snapshot or '/path/to/Sana_Sprint_0.6B_1024px_diffusers'} "
        "--output /tmp/sanasprint-mlx-real-block-denoise.json "
        "--dtype bfloat16 --sample-size 2 --prompt-sequence-length 4 --block-count 2"
    )
    if gate_id in evidence["gates"]:
        item = dict(evidence["gates"][gate_id])
        return {"id": gate_id, "status": "PASS", "reason": "external pass evidence", "command": command, **item}
    ...
    if _looks_remote(snapshot):
        return {
            "id": gate_id,
            "status": "BLOCKED",
            "reason": "real block denoise requires a local snapshot path",
            "command": command,
        }
    exists = Path(snapshot).exists()
    return {
...
def _real_transformer_loop_gate(snapshot, evidence: dict) -> dict:
    gate_id = "real_transformer_loop"
    command = (
        "sanasprint-mlx-verify real-transformer-loop "
        f"--snapshot {snapshot or '/path/to/Sana_Sprint_0.6B_1024px_diffusers'} "
        "--output /tmp/sanasprint-mlx-real-transformer-loop.json "
        "--dtype bfloat16 --steps 1 --sample-size 2 --prompt-sequence-length 4 --block-count 2"
    )
    if gate_id in evidence["gates"]:
        item = dict(evidence["gates"][gate_id])
        return {"id": gate_id, "status": "PASS", "reason": "external pass evidence", "command": command, **item}
    ...
    if _looks_remote(snapshot):
        return {
            "id": gate_id,
            "status": "BLOCKED",
            "reason": "real transformer loop requires a local snapshot path",
            "command": command,
        }
    exists = Path(snapshot).exists()
    return {
